...
int main() {
    ...
    string S;
    vector<vi> S_map(N, vi(10,-1));
    vi S_cost(10, 1000000);
    bool jag=false;
    int old_A;
    rep (i, N) {
        int A;
        ...
        rep (j,A) {
            ...
            int tmp=0;
            if(i==0){
                rep(z,end_p,static_cast<int>(T.length())){
                    if(T[z]==S[tmp]){
                        tmp++;
                    }else{
                        if(tmp==0){
                ...
                bool flag=false;
                rep(y,old_A){
                    rep(z,S_map[i-1][y],static_cast<int>(T.length())){
                        if(T[z]==S[tmp]){
                            tmp++;
                        }else{
                            if(tmp!=0){
                            ...
                            S_map[i][j]=z;
                            if(static_cast<int>(T.length()) - 1==z)jag=true;
                            break;
                        }
                    }
                    if(flag)break;
    ...
    if(jag){
        cout << *min_element(S_cost.begin(), S_cost.end());
    }else{
        cout << -1;
    }
