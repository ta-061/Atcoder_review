...
ll dfs(int p, bool is, bool has, int maxd, const string& num, vector<vvvll>& dp) {
    if (static_cast<size_t>(p)==num.size()) {
        return has ? 1:0;
    }
    if (dp[p][is][has][maxd] != -1) {
        return dp[p][is][has][maxd];
    }
    ll res = 0;
    int lim = is ? (num[p]-'0'):9;
    rep(dig, lim+1){
        if(has&&dig>=maxd)continue;
        res+=dfs(p+1, is&&(dig==lim), has||(dig>0), has?max(maxd,dig):dig, num, dp);
    }
    return dp[p][is][has][maxd] = res;
}
...
int main() {
    string L, R;
    cin >> L >> R;
    vector<vvvll> dpR(R.size(), vvvll(2, vvll(2, vll(10, -1))));
    ll countR = dfs(0, true, false, 0, R, dpR);
    string L_minus_1 = to_string(stoll(L) - 1);
    vector<vvvll> dpL(L_minus_1.size(), vvvll(2, vvll(2, vll(10, -1))));
    ll countL = dfs(0, true, false, 0, L_minus_1, dpL);
