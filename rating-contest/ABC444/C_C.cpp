...
bool jage(const vll& A, ll L){
    unordered_map<ll,int> cnt;
    cnt.reserve(A.size()*2);
    for(ll x:A)if(x!=L)cnt[x]++;
    for(auto &p: cnt){
        ll y=L-p.first;
        if(p.first>y) continue;
        int cy=cnt.count(y) ? cnt[y]:0;

        if(p.first == y){
            if(p.second%2)return false;
        }else{
            if(p.second != cy)return false;
        }
    }
    return true;
}
...
int main() {
    ...
    sort(all(A));
    vll ans;
    if(jage(A,A[N-1]))ans.pb(A[N-1]);
    if(jage(A,A[N-1]+A[0]))ans.pb(A[N-1]+A[0]);
