...
vv<char> rotate(const vv<char>& mat) {
    int N = mat.size();
    vv<char> res(N, vc<char>(N));
    rep(i, N) rep(j, N) {
        res[j][N - 1 - i] = mat[i][j];
    }
    return res;
}
int count_diff(const vv<char>& s, const vv<char>& t) {
    int N = s.size();
    int cnt = 0;
...
int main() {
    ...
    int N;
    cin >> N;
    vv<char> S(N, vc<char>(N)), T(N, vc<char>(N));
    rep(i,N)rep(j,N)cin >> S[i][j];
    rep(i,N)rep(j,N)cin >> T[i][j];
    int ans = count_diff(S,T);
    auto r1=rotate(S);
    auto r2=rotate(r1);
    auto r3=rotate(r2);
    chmin(ans,count_diff(r1,T)+1);
    chmin(ans,count_diff(r2,T)+2);
    chmin(ans,count_diff(r3,T)+3);
    cout << ans << endl;
