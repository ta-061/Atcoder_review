...
vvb adj;
int ans=0;
void dfs(int index, int left, vi& deg, int depth) {
    if(edges.size()-index<left)return;
    if(depth+left<ans)return;
    if(left==0){
        rep(i,N)if(deg[i]!=2)return;
        chmax(ans,depth);
        return;
    }
    if(index==edges.size())return;
    dfs(index+1, left, deg, depth);

    auto[u,v] = edges[index];
    if(deg[u]<2 && deg[v]<2){
        deg[u]++;
        deg[v]++;
        dfs(index+1, left-1, deg, depth+(adj[u][v] ?1:0));
        deg[u]--;
        deg[v]--;
    }
}
