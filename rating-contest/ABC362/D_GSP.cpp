...
vll dijkstra(ll s,const vector<H>& UVB,const vll& A){
    ll n=A.size()-1;
    vector<vector<pll>> g(n+1);

    for(const auto& edge:UVB){
        g[edge.U].emplace_back(edge.V, edge.B);
        g[edge.V].emplace_back(edge.U, edge.B);
    }
    ...
    while (!pq.empty()) {
        ...
        for (const auto& nei: g[u]) {
            ll v = nei.first;
            ll cost = nei.second+ A[v];
...
int main() {
    ...
    vector<H> UVB(M);
    rep(i, M){
        cin >> UVB[i].U >> UVB[i].V >> UVB[i].B;
    }
    vll ans=dijkstra(1,UVB,A);
    rep(i,2,N+1){
        cout << ans[i] << " ";
    }
    return 0;
