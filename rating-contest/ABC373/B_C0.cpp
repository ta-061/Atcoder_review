...
int main() {
    string S;
    cin >> S;
    string map_s = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    int s=distance(S.begin(),find(all(S), map_s[0]));
    int ans=0;
    rep(i,1, S.size()){
        int e=distance(S.begin(),find(all(S),map_s[i]));
        cout << S[e] << endl;
        //if(s<e)ans+=e-s;
        //else ans+=26-(s-e);
        //ans+=min(abs(e-s),26-abs(e-s));
        ans+=abs(e-s);
        //cout << ans <<"start"<<s<<"end"<<e<<S[e]<< endl;
        s=e;
    }
    cout << ans;
}
