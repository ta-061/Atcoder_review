...
bool upper(const pll& v){
    return(v.first>0 && v.second==0)||(v.second>0);
}

bool ang_less(const pll& a, const pll& b){
...
int main() {
    ...
    vector<pll> dir;
    dir.reserve(cnt.size());
    for(auto &k:cnt)dir.push_back(k.first);
    sort(dir.begin(),dir.end(),ang_less);

    int M=dir.size();
    map<pll,int>dx;
    rep(i,M)dx[dir[i]]=i;
    vll addf(M+1,0);
    rep(i,M)addf[i+1]=addf[i]+cnt[dir[i]];
    vll c(M);
    rep(i,M)c[i]=cnt[dir[i]];
    vll paddf(2*M+1,0);
    rep(i,2*M)paddf[i+1]=paddf[i]+c[i%M];
    ...
    rep(i,Q){
        ...
        int r=dx[mo[A]];
        if(r<dx[mo[B]])r+=M;
        cout<<paddf[r+1]-paddf[dx[mo[B]]]<<endl;
    }
