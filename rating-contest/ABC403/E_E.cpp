...
int main(){
    ...
    struct XNode {
        XNode* child[26];
        bool isEnd;
        XNode() : isEnd(false) { fill(child, child+26, nullptr); }
    };
    XNode* rootX = new XNode();

    struct YNode {
        YNode* child[26];
        int subtree;
        YNode() : subtree(0) { fill(child, child+26, nullptr); }
    };
    YNode* rootY = new YNode();
    ...
    auto insertY = [&](const string& s){
        YNode* u = rootY;
        u->subtree++;
        for(char ch : s){
            int c = ch - 'a';
            if(!u->child[c]) u->child[c] = new YNode();
            u = u->child[c];
            u->subtree++;
        }
    };

    auto removeYprefix = [&](const string& s){
        YNode* u = rootY;
        vector<YNode*> path;
        path.push_back(u);
        for(char ch : s){
    ...
    auto insertX = [&](const string& s){
        XNode* u = rootX;
        bool hasShorter = u->isEnd;
        for(char ch : s){
            int c = ch - 'a';
            if(!u->child[c]) u->child[c] = new XNode();
            u = u->child[c];
            if(u->isEnd) hasShorter = true;
        }
        if(u->isEnd) return;
        if(!hasShorter){
            removeYprefix(s);
