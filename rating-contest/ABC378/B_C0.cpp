...
int main() {
    int N, Q;
    cin >> N;
    vector<vector<int>> qr_map(N+1, vector<int>(2));
    rep(i,1, N+1)cin >> qr_map[i][0] >> qr_map[i][1];
    cin >> Q;
    int t, d, tmp;
    rep(i,Q){
        cin >> t >> d;
        tmp = d%qr_map[t][0];
        if(tmp == qr_map[t][1])tmp = d;
        else if(tmp-qr_map[t][1] < 0)tmp = qr_map[t][1]-tmp+d;
        else tmp = qr_map[t][1]+qr_map[t][0]-tmp+d;
        cout << tmp << endl;
    }
}
