...
struct node{
    int nxt[10];
    bool end=false;
    node() {fill(nxt,nxt+10,-1);}
};

vector<node> tr(1);
map<pair<int,vi>,ll> memo;
...
vi go(const vi& st, int d){
    vi nx;
    for(int v:st){
        if(tr[v].nxt[d]!=-1) nx.pb(tr[v].nxt[d]);
        if(tr[v].end && tr[0].nxt[d]!=-1) nx.pb(tr[0].nxt[d]);
    }
    sort(all(nx));
...
ll dfs(int rem, const vi& st){
    if(st.empty())return 0;
    if(rem==0){
        for(int v:st)if(tr[v].end)return 1;
        return 0;
    }
    auto key = make_pair(rem,st);
