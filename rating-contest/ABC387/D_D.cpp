...
int main() {
    ...
    while(!q.empty()){
        ...
        rep(i,n){
            ...
            if (LR) {
                rep(j, 2) {
                    int nx=cur.first+dx[j*2];
                    int ny=cur.second;
                    if (out_grid(nx, ny, H, W)) continue;
                    if (S[nx][ny] == '#') continue;
                    if (visited.count({{nx, ny}, false})) continue;
                    visited.insert({{nx, ny}, false});
                    q.push({{nx, ny}, false});
                }
            } else {
                rep(j, 2) {
                    int nx=cur.first;
                    int ny=cur.second+dy[j*2+1];
                    if (out_grid(nx, ny, H, W)) continue;
                    if (S[nx][ny] == '#') continue;
                    if (visited.count({{nx, ny}, true})) continue;
