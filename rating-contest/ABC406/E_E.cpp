...
int main() {
    ...
    rep(t,T){
        ...
        vector<array<ll,2>> dpcon(K+1),dpsum(K+1), ndpcon(K+1), ndpsum(K+1);
        rep(i,K+1){
            dpcon[i][0]=0;
            dpcon[i][1]=0;
            dpsum[i][0]=0;
        ...
        rep(i,60){
            ...
            rep(c,K+1){
                rep(t,2){
                    ...
                    if(ws == 0) continue;
                    int nt = (t && bit[i]==0);
                    ndpcon[c][nt] = (ndpcon[c][nt] + ws) % MOD;
                    ndpsum[c][nt] = (ndpsum[c][nt] + ss) % MOD;
                    if(c+1 <= K && (!t || bit[i]==1)){
                        int nt = (t && bit[i]==1);
                        ndpcon[c+1][nt] = (ndpcon[c+1][nt] + ws) % MOD;
                        ll add = (ss + ws * w) % MOD;
                        ndpsum[c+1][nt] = (ndpsum[c+1][nt] + add) % MOD;
                    }
                }
            }
            swap(dpcon, ndpcon);
        ...
        ll ans = (dpsum[K][0]+dpsum[K][1]) % MOD;
        cout << ans << endl;
    }
